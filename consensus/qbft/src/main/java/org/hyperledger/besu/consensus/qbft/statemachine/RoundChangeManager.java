...
public class RoundChangeManager {
  ...
  public Optional<Integer> futureRCQuorumReceived(final ConsensusRoundIdentifier currentRoundIdentifier) {
    // Iterate through elements of round summary, identify ones with round number higher than current,
    ...
    Map<Address, Integer> higherRounds = roundSummary.entrySet().stream()
            .filter(entry -> isAFutureRound(entry.getValue(),currentRoundIdentifier))
            .collect(Collectors.toMap(Map.Entry::getKey, entry -> entry.getValue().getRoundNumber()));
